...
const Header = () => {
 ...
 // ✅ 알림 로딩 및 SSE 연결 통합 로직
  useEffect(() => {
    // 1. 로그인 상태이고 관리자가 아닐 때만 실행
    if (loggedIn && !isAdmin) {
      ...
      const eventSource = new EventSource(`http://localhost:8080/api/notifications/subscribe?loginId=${loginId}`);

      eventSource.onopen = () => {
        console.log("✅ 실시간 알림 서버와 연결되었습니다!");
      };

      // 서버에서 발송하는 'notification' 이벤트 수신
      eventSource.addEventListener("notification", (event) => {
        try {
          const newNoti = JSON.parse(event.data);
          console.log("🔔 실시간 알림 도착:", newNoti);
          setNotifications((prev) => [newNoti, ...prev]); // 새 알림을 맨 앞에 추가
        } catch (err) {
      ...
      // [C] 정리(Clean-up): 컴포넌트가 사라지거나 로그아웃 시 연결 종료
      return () => {
        console.log("🔌 SSE 연결을 안전하게 종료합니다.");
        eventSource.close();
      };
    }
  }, [loggedIn, isAdmin]); // 의존성 배열 확인
