...
interface StationSidebarProps {
  ...
  isSearchMode: boolean;               // ✅ 추가
  setIsSearchMode: (v: boolean) => void; // ✅ 추가
  isLoading: boolean;
  speedFilter: string;
    ...
    <div className="w-full min-w-[380px] h-full bg-white flex flex-col overflow-hidden border-r border-gray-100 shadow-[20px_0_30px_rgba(0,0,0,0.03)] z-20">
      <div className="p-5 border-b space-y-5 shrink-0">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex gap-2">
              {['전체', '여유', '혼잡'].map((t) => (
                <button key={t} onClick={() => setStatusFilter(t)} className={`px-5 py-2 rounded-full text-xs font-semibold border transition-all ${statusFilter === t ? 'bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-100' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}> {t} </button>
              ))}
            </div>
          </div>
          <div className="space-y-2.5">
      ...
      <div className="px-5 py-3.5 bg-blue-50/50 border-b flex items-center justify-between shrink-0">
        <p className="text-blue-700 text-[13px] font-bold flex items-center gap-1.5">
          ...
          {isSearchMode ? `'${keyword}' 검색 결과 ${stations.length}개` : `내 위치 반경 1.5km 안 충전소 ${stations.length}개`}
        </p>
        {isSearchMode && (
          <button onClick={() => { setKeyword(''); setSearchResults([]); setIsSearchMode(false); }} className="text-[10px] text-gray-400 hover:text-red-500 underline">초기화</button>
        )}
      </div>
